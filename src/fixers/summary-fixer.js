...
export default class SummaryFixer extends Fixer {
    ...
    _createPackagesSummaryElement(backupDocument) {
        ...
        if (data) {
            let links = data.querySelectorAll("a");
            let count = this._parseCountFromMoreLink(links);
    ...
    _createReleasesSummaryElement(backupDocument, location) {
        ...
        if (data) {
            let links = data.querySelectorAll("a");
            count = this._parseCountFromMoreLink(links);
            link = links[0].href;
        } else {
            count = 0;
    ...
    _createContributorsSummaryElement(backupDocument, location) {
        ...
        if (data) {
            count = data.querySelector("span.Counter").innerText;
    ...
    // Fix:
    // Probably wrong result (0) for 1 release/package;
    // Probably NaN for [1000; +∞);
    _parseCountFromMoreLink(links) {
        if (links.length) {
            let match = links[links.length - 1].innerText.match(/\+ ([\d]+).*/);
            if (match)
                return +match[1] + 1;
        }
        return 0;
