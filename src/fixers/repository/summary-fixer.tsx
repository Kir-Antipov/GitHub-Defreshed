...
export default class SummaryFixer extends Fixer {
     ...
     */
    _createCommitsSummaryElement(backupContainer: HTMLElement) {
        let data = backupContainer.querySelector<HTMLAnchorElement>("#backup-commits");
     ...
     */
    _createBranchesSummaryElement(backupContainer: HTMLElement) {
        let data = backupContainer.querySelector<HTMLAnchorElement>("#backup-branches");
        let count = data.querySelector("strong").innerText;
        let link = data.href;
     ...
     */
    _createPackagesSummaryElement(location: string) {
     ...
     */
    _createReleasesSummaryElement(location: string, treatTagsAsReleases: boolean, showLatestReleasePopup: boolean) {
        let svg = `<svg class="octicon octicon-tag" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path fill-rule="evenodd" d="M2.5 7.775V2.75a.25.25 0 01.25-.25h5.025a.25.25 0 01.177.073l6.25 6.25a.25.25 0 010 .354l-5.025 5.025a.25.25 0 01-.354 0l-6.25-6.25a.25.25 0 01-.073-.177zm-1.5 0V2.75C1 1.784 1.784 1 2.75 1h5.025c.464 0 .91.184 1.238.513l6.25 6.25a1.75 1.75 0 010 2.474l-5.026 5.026a1.75 1.75 0 01-2.474 0l-6.25-6.25A1.75 1.75 0 011 7.775zM6 5a1 1 0 100 2 1 1 0 000-2z"></path></svg>`;
        ...
        if (showLatestReleasePopup && element) {
            ...
            let latestReleaseDate = section && section.querySelector<HTMLElement>("relative-time");
     ...
     */
    _createContributorsSummaryElement(location: string) {
    ...
    _createLicenseSummaryElement() {
        ...
        if (data) {
            let anchor = data.parentElement as HTMLAnchorElement;
            let link = anchor.href;
            let text = anchor.innerText.trim().replace(" License", "");
     ...
     * @param svg Element's icon.
     * @param text Element's text.
     * @param pluralEnding Plural ending for the text value.
     ...
     */
    _createSummaryElementFromRightBar(location: string, svg: string, text: string, pluralEnding = "s", defaultCount: number | string = 0, additionalPath = "", force = true, useSecondaryCount = false) {
     ...
     */
    _findRightBarSectionByName(name: string) {
     ...
     *
     * @param svg Element's icon.
     * @param link Element's link.
     * @param text Element's text.
     * @param count Element's count.
     * @param pluralEnding Plural ending for the text value.
     */
    _createSummaryElement(svg: string, link: string, text: string, count: number | string = -1, pluralEnding = "s") {
        return (
            <li>
                <a href={link}>
